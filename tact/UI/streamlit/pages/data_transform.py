...
def app():
    ...
    target_columns = st.multiselect(label="Select target columns:", options=df.columns)
    ...
    with col2:
        constants = st.text_input(label="Define constants as key/value pairs:", value={'key1': 'value1', 'key2': 'value2', 'key3': 0})


    drop_units = st.checkbox(label='Drop first row (units)')
    drop_empty_records = st.checkbox(label='Drop records with a value of 0 or less in target columns')

    output_path = st.text_input(label="Output path:", placeholder='path/to/output.csv')
    ...
    constants_table = dt.DictTable(constants, 'Constants:')
    ...
    if st.button(label="Flip It!"):
        with st.spinner('Processing...'):
            if append_constants:
                controller.flip_dataset(target_columns, output_path, drop_units, drop_empty_records, constants)
            else:
                controller.flip_dataset(target_columns, drop_units, drop_empty_records, output_path)
    ...
    combine_output_path = st.text_input(key='row_combine_output', label="Output path:", placeholder='path/to/output.csv')
