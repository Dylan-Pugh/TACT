...
def create_iso_time(csv_row, date_fields, time_field):
    ...
    try:
        ...
        logger.error(f"Manual time parsing failed: {e}")
        logger.error(f"Input row: {csv_row}")
        logger.error(f"Time fields: {time_field}")
...
def compile_datetime(
    ...
    with open(outfile, "w") as outfile:
        writer = csv.DictWriter(outfile, delimiter=",", fieldnames=out_flds)
        writer.writeheader()
        ...
        for csv_row in reader:
            try:
                ...
                logger.error(f"Input row: {csv_row}")
                logger.error(f"Output row: {orow}")
